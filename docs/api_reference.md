...
- Example request:
  - fetch('/api/auth/login', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-csrf-token': '...' }, body: JSON.stringify({ identifier: 'user', password: 'secret' }) })
- Example response:
...
- Example request:
  - fetch('/api/auth/register', { method: 'POST', headers: { 'Content-Type':'application/json','x-csrf-token':'...' }, body: JSON.stringify({ email:'e@x.com', password:'secret', username:'user' }) })
- Example response:
...
- Example request:
  - fetch('/api/media-cache?url=https://example.com/image.jpg')
- Example response:
...
- Example response:
  - 302 redirect to /media-cache/...

### /api/revalidate
- Methods: POST
- Description: Revalidates Next.js tags/pages using a shared secret.
- Example request:
  - fetch('/api/revalidate?secret=...', { method:'POST', body: JSON.stringify({ slug:'post-slug' }) })
- Example response:
...
- Example request (GET):
  - fetch('/api/wp/comments?post=123&per_page=20')
- Example response (GET):
  - [ { id: 10, post: 123, author_name: 'A', content: { rendered: '...' } } ]
- Example request (POST):
  - fetch('/api/wp/comments', { method:'POST', headers:{ 'Content-Type':'application/json' }, body: JSON.stringify({ postId:123, content:'Nice post!' }) })
...
- Example request:
  - fetch('/api/wp/media/list?per_page=20')
- Example response:
...
- Example request (GET):
  - fetch('/api/wp/posts?per_page=10&_embed=1')
- Example response (GET):
...
- Example request:
  - fetch('/api/wp/related?categories=1,2&tags=3&exclude=10&per_page=5')
- Example response:
...
- Example request:
  - fetch('/api/wp/track-view', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ postId: 123 }) })
- Example response:
...
- Example request:
  - fetch('/api/wp/users?search=john&per_page=10')
- Example response:
