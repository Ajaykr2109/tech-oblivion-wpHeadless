...
export async function GET(req: Request) {
  try {
    ...
    return new Response(JSON.stringify({ error: 'server_error', detail: String(e instanceof Error ? e.message : e) }), { status: 500, headers: { 'Content-Type': 'application/json' } })
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    const postId = Number((body && typeof body === 'object' && 'postId' in body ? body.postId : undefined) ?? (body && typeof body === 'object' && 'post_id' in body ? body.post_id : undefined) ?? 0)
